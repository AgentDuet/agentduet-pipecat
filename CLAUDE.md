...
The inbound spike is **complete and merged** (2026-08-11): `AgentDuetTransport` (inbound only), a keyless example bot, an outbound track probe, and a test suite pinning the teardown matrix. Live-validation results — barge-in measurements, the resolved outbound-track question, the abandoned-ring observation — are recorded in [specs/2026-08-03-agentduet-transport-spike-design.md](specs/2026-08-03-agentduet-transport-spike-design.md) under "Spike results". The parent spec is [specs/pipecat-transport-spec.md](specs/pipecat-transport-spec.md) (Draft v3; its code sketches are conceptual and predate the spike — where they disagree with the code, the code is right).
